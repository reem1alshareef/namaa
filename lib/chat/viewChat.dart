...
class _viewChatState extends State<viewChat> {
  ...
  viewModalChat _viewModelChat = viewModalChat();
  ...
  void _sendMessage(String text) async {
    ...
    if (text == '7') {
      ...
      // User selected a choice, generate a specific response
      String response = generateResponse(text);
      _receiveChatGPTResponse(response);
    } else {
      // User entered something other than choices, send to ChatGPT
      String answer = await _viewModelChat.getChatGPTAnswer(text);
      _receiveChatGPTResponse(answer);
    }
  }
  ...
  void _receiveChatGPTResponse(String response) {
    DateTime now = DateTime.now();
    setState(() {
      messages.insert(0, Message(text: response, isUser: false, time: now));
    });
